...
def main():
	...
	"It is necessary to specify the dockerbench path with the command line argument or environment variable. If the application has a REST API, "
	...
	"it is also required to point out the container port number to test, and the --https option if the application supports it")
	
	parser.add_argument('--name', type=str, metavar="string", help="Container name (it has to already be running)", required=True)  
